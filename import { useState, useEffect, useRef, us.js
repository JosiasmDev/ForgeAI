...
const TOOLS_DEF = {
  ...
  code_runner: { id:'code_runner', name:'Code Runner', category:'qa', description:'Ejecuta código en sandbox seguro.', permissions:['sandbox'], timeout:30000, maxRetries:1, available:true,
    ...
    execute: async ({code,language}, ctx) => { await sd(280); let out=''; if ((language==='javascript'||language==='typescript')&&!code.includes('import')&&!code.includes('fetch')&&code.length<500) { try { const r=new Function('"use strict";return('+code.trim()+')')(); out=`Output: ${JSON.stringify(r)}\nExit: 0`; } catch(e) { return mkR(false,`Error: ${e.message}`,300,'code_runner',null,e.message); } } else { out=`[Sandbox/${language}] Ejecutado\nExit: 0`; } return mkR(true,out,350,'code_runner',{language,exitCode:0}); } },
...
function Btn({ onClick, children, variant='primary', size='md', disabled, style:st={} }) {
  ...
  return <button onClick={onClick} disabled={disabled} style={{ ...V[variant], ...S[size], borderRadius:8, fontWeight:500, opacity:disabled?0.5:1, ...st }} onMouseEnter={e=>!disabled&&(e.target.style.filter='brightness(1.12)')} onMouseLeave={e=>e.target.style.filter='none'}>{children}</button>;
}
